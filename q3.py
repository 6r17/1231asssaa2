...
for i, _post in enumerate(snscrape.modules.twitter.TwitterSearchScraper('(about:{}) since_time:{}'.format(keyword, int(search_date.timestamp()))).get_items()):
	...
	if tr_post['hashtags']:
		for hashtag in tr_post['hashtags']:
			if hashtag.lower() not in unique_hashtags:
				unique_hashtags.append(hashtag.lower())
	...
	elements.append(tr_post)

pd_dataframe = pd.DataFrame(elements)
# print(pd_dataframe)
print(len(unique_hashtags), 'unique hashtags')